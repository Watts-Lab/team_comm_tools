    ...
    def __init__(self, chat_data: pd.DataFrame, 
        ...
        if convo_columns is None:
            ...
            if convo_aggregation == True and (len(convo_columns) == 0 or len(convo_methods) == 0):
                ...
                # to check if columns are in the list of features we generate
                ...
                if(len(convo_columns_in_features) != len(convo_columns)):
                    ...
                    for i in convo_columns:
                        matches = process.extract(i, self.chat_features, limit=3)
                        best_match, similarity = matches[0]
                        ...
                        if similarity == 100:
                            ...
                            print(i, "not found in feature list and no close match.")

                # user_columns_in_features and user_columns_in_data should be the same    
        ...
        if user_columns is None:
            ...
            if user_aggregation == True and len(user_columns) == 0:
                ...
                # to check if columns are in the list of features we generate
                user_columns_in_features = list(set(user_columns).intersection(set(self.chat_features)))
        ...
        for i in range(len(self.convo_methods)):
            ...
            if current != "mean" and current != "max" and current != "min" and current != "stdev" and current != "median":
                print("Warning: ", current, "is not a valid convo method. Ignoring them.")
                self.convo_methods.remove(current)
