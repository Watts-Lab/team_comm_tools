...
class ConversationLevelFeaturesCalculator:
    ...
    def get_temporal_features(self) -> None:
        if {'timestamp'}.issubset(self.chat_data.columns):
            self.conv_data = pd.merge(
                left=self.conv_data,
                right=get_mean_msg_duration(self.chat_data, "timestamp"),
                on=["conversation_num"],
                how="inner"
            )
            self.conv_data = pd.merge(
                left=self.conv_data,
                right=get_stddev_msg_duration(self.chat_data, "timestamp"),
                on=["conversation_num"],
                how="inner"
            )
        else:
            print("No timestamp detected, skipping temporal features...")
    ...
    def get_conversation_level_summary_statistics_features(self) -> None:
       ...
       # Content word mimicry
        ...
        # Textblob Polarity
        # Min and Max for Polarity
        self.conv_data = pd.merge(
            left=self.conv_data,
            right=get_average(self.chat_data, 'textblob_polarity', 'average_textblob_polarity'),
            on=["conversation_num"],
            how="inner"
        )
        ...
        self.conv_data = pd.merge(
            left=self.conv_data,
            right=get_stdev(self.chat_data, 'textblob_polarity', 'std_textblob_polarity'),
            on=["conversation_num"],
            how="inner"
        )
        ...
        self.conv_data = pd.merge(
            left=self.conv_data,
            right=get_average(self.chat_data, 'textblob_subjectivity', 'average_textblob_subjectivity'),
            on=["conversation_num"],
            how="inner"
        )
        ...
        self.conv_data = pd.merge(
            left=self.conv_data,
            right=get_stdev(self.chat_data, 'textblob_subjectivity', 'std_textblob_subjectivity'),
            on=["conversation_num"],
            how="inner"
        )   
    ...
    def get_talkative_member_features(self) -> None:
        ...
        self.conv_data = pd.merge(
            left=self.conv_data,
            right=get_max(self.chat_data, 'textblob_polarity', 'max_textblob_polarity'),
            on=["conversation_num"],
            how="inner"
        )
        ...
        self.conv_data = pd.merge(
            left=self.conv_data,
            right=get_min(self.chat_data, 'textblob_polarity', 'min_textblob_polarity'),
            on=["conversation_num"],
            how="inner"
        )

        # Min and Max for Subjectivity
        self.conv_data = pd.merge(
            left=self.conv_data,
            right=get_max(self.chat_data, 'textblob_subjectivity', 'max_textblob_subjectivity'),
            on=["conversation_num"],
            how="inner"
        )
        ...
        self.conv_data = pd.merge(
            left=self.conv_data,
            right=get_min(self.chat_data, 'textblob_subjectivity', 'min_textblob_subjectivity'),
            on=["conversation_num"],
            how="inner"
        )
