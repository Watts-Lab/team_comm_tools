...
'''
These functions are for the purpose of calculating aggregates of various chat level features at the conversation level.
...
def get_average(chat_level_data, column_to_summarize, new_column_name):
	conversation_data = chat_level_data.copy()
	conversation_data[new_column_name] = conversation_data.groupby(["conversation_num"], sort=False)[column_to_summarize].transform(lambda x: np.mean(x))
	return(conversation_data[["conversation_num", new_column_name]].drop_duplicates())
...
def get_max(chat_level_data, column_to_summarize, new_column_name):
	conversation_data = chat_level_data.copy()
	conversation_data[new_column_name] = conversation_data.groupby(["conversation_num"], sort=False)[column_to_summarize].transform(max)
	return(conversation_data[["conversation_num", new_column_name]].drop_duplicates())
...
def get_min(chat_level_data, column_to_summarize, new_column_name):
	conversation_data = chat_level_data.copy()
	conversation_data[new_column_name] = conversation_data.groupby(["conversation_num"], sort=False)[column_to_summarize].transform(min)
	return(conversation_data[["conversation_num", new_column_name]].drop_duplicates())
...
def get_stdev(chat_level_data, column_to_summarize, new_column_name):
	conversation_data = chat_level_data.copy()
	conversation_data[new_column_name] = conversation_data.groupby(["conversation_num"], sort=False)[column_to_summarize].transform(lambda x: np.std(x))
	return(conversation_data[["conversation_num", new_column_name]].drop_duplicates())
...
def get_sum(chat_level_data, column_to_summarize, new_column_name):
	conversation_data = chat_level_data.copy()
	conversation_data[new_column_name] = conversation_data.groupby(["conversation_num"], sort=False)[column_to_summarize].transform(lambda x: np.sum(x))
	return(conversation_data[["conversation_num", new_column_name]].drop_duplicates())
