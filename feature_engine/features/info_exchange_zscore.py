...
def get_zscore_across_all_conversations(chats_data, on_column):
  chats_data['info_exchange_zscore_conversation'] = chats_data[["conversation_num", on_column]].groupby(["conversation_num"])[on_column].transform(lambda x : stats.zscore(x))
  return chats_data
